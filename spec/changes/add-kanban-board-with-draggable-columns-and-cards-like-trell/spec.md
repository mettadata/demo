...
## ADDED: Requirement: kanban-data-model

The application MUST introduce a `KanbanStore` backed by a Svelte writable store that tracks an ordered list of columns. Each column MUST have a unique `id` (string), a `title` (string), and an `order` (number). The store MUST also track an ordered mapping of todo IDs to columns and positions within those columns. The default column set MUST be "To Do", "In Progress", and "Done". Any todo not yet assigned to a column MUST be placed in the first column automatically.
...
- GIVEN localStorage contains a valid `kanban-state` key
- WHEN the KanbanStore initializes
...
## ADDED: Requirement: card-drag-and-drop-between-columns
...
## ADDED: Requirement: card-reordering-within-column
...
- THEN the `kanban-state` key in localStorage MUST contain a JSON value reflecting the updated column assignments

### Scenario: persist-view-preference
- GIVEN the user is viewing the list view
- WHEN the user toggles to the kanban board view
- THEN the `view-preference` key in localStorage MUST be set to "kanban"

### Scenario: restore-view-preference-on-load
- GIVEN localStorage `view-preference` is set to "kanban"
- WHEN the application loads
- THEN the kanban board view MUST be displayed instead of the list view

---

## ADDED: Requirement: view-toggle
...
## ADDED: Requirement: board-component-structure

The kanban feature MUST be implemented with three primary Svelte components: `KanbanBoard.svelte` (renders the full board with horizontal column layout), `KanbanColumn.svelte` (renders a single column with its header and list of cards), and `KanbanCard.svelte` (renders a single card displaying todo title, description preview, and priority indicator). Each component MUST accept its data via props from the parent component.
...
## ADDED: Requirement: accessibility-for-drag-and-drop

All draggable cards MUST have `role="listitem"` and columns MUST have `role="list"`. Cards MUST include `aria-grabbed` and `aria-dropeffect` attributes that update during drag operations. The board SHOULD provide keyboard-accessible card movement as an alternative to mouse drag-and-drop. Each column MUST have an accessible label derived from its title.
...
- THEN `aria-grabbed` MUST change from "false" to "true" on the card AND the target column MUST have `aria-dropeffect` set to "move"

---

## ADDED: Requirement: unit-tests-for-kanban-store
...
- WHEN the `addColumn("Review")` function is called
...
- WHEN `moveCard(cardId, "In Progress", 0)` is called
...
- WHEN `deleteColumn("Review")` is called
