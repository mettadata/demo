...
- THEN bcrypt comparison MUST succeed, a new session entry MUST be created in the server-side session store keyed by a generated session ID, an httpOnly SameSite=Strict cookie named  MUST be set to that session ID, and the response MUST redirect to the main todo route
...
The application MUST provide a logout mechanism implemented as a SvelteKit form action (not a GET route). When the action is invoked the server MUST delete the session record from the server-side session store, clear the  cookie by setting it with an expired , and redirect the user to .

### Scenario: successful-logout
- GIVEN a user is authenticated with a valid session cookie
- WHEN the user submits the logout form action
- THEN the session record MUST be removed from the server-side session store, the  cookie MUST be cleared (Max-Age=0 or equivalent), and the response MUST redirect to

### Scenario: logout-invalidates-session
- GIVEN a user has logged out and their former session ID is known
- WHEN any subsequent request is made with the former  cookie value
- THEN  MUST NOT resolve the session to a user,  MUST be undefined, and the request MUST be redirected to


## Requirement: session-enforcement

The application MUST implement session enforcement in . On every request the hook MUST read the  cookie, look up the corresponding session in the server-side session store, and attach the resolved user object to . If the cookie is absent or the session ID does not resolve to a valid session,  MUST be . Every route except  and  MUST redirect unauthenticated requests to . The  type MUST be extended in  to declare the  field.

### Scenario: authenticated-request-passes-through
- GIVEN a request is made to the main todo route  with a valid  cookie that resolves to user "alice"
- WHEN  processes the request
- THEN  MUST be populated with "alice"'s user object and the route load function MUST execute normally without redirecting

### Scenario: unauthenticated-request-redirected
- GIVEN a request is made to  with no  cookie
- WHEN  processes the request
- THEN  MUST be  and the response MUST be a redirect to

### Scenario: invalid-session-id-redirected
- GIVEN a request is made to  with a  cookie containing a value not present in the session store
- WHEN  processes the request
- THEN  MUST be  and the response MUST be a redirect to

### Scenario: public-routes-not-blocked
- GIVEN a request is made to  or  with no  cookie
- WHEN  processes the request
...
The server-side in-memory todo store MUST be a  keyed by . All load functions and form actions that read or mutate todos MUST scope their operations to . A user MUST only ever see and modify their own todos. Two different authenticated users operating simultaneously MUST not observe each other's todo lists.
...
- THEN the stored credential MUST be a bcrypt hash string (beginning with  or ) and MUST NOT equal the plaintext string "securepassword"

### Scenario: login-uses-bcrypt-compare
- GIVEN the user store contains "alice" with a bcrypt hash of "securepassword"
- WHEN "alice" logs in with the correct password "securepassword"
- THEN the server MUST call  and rely solely on its boolean result to grant or deny access, never performing string equality against the hash

### Scenario: incorrect-password-fails-bcrypt-compare
- GIVEN the user store contains "alice" with a bcrypt hash of "securepassword"
- WHEN a login attempt is made with password "wrongpassword"
- THEN  MUST return false, the login MUST be rejected, and no session MUST be created
